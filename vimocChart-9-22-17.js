...
//  freeboard.addStyle(".focus circle","fill:none; stroke:white");
  ...
  freeboard.loadWidgetPlugin({
    "type_name": "vimocChartWidget",
    "display_name": "Vimoc Historical Chart",
    "description": "Creates a multi-series line chart from Vimoc Datasource data.",
    "external_scripts": [
    ...
    {
      newInstanceCallback(new vimocChartWidget(settings));
    }
  });

  var vimocChartWidget = function(settings)
  {
    var self = this;
    ...
    this.onCalculatedValueChanged = function(settingName, newValue)
    {
      ...
      { 
        // Set the data:
        var data = processDataNVD3(newValue);
...
//                 break;
    ...
    function processDataNVD3(vimocdata) {
    	...
    	vimocdata["sensors"].forEach(function(sensor) {
    		if(sensor["data"]["history"].length > 0) {
				var inobj = {"values":[]};
				var outobj = {"values":[]};
				inobj["key"] = sensor["info"]["Name"] + "-" + capitalizeFirstLetter(sensor["info"]["in"]);
				...
				outobj["key"] = sensor["info"]["Name"] + "-" + capitalizeFirstLetter(sensor["info"]["out"]);
    ...
    function createLineChart(data,axis,ymin,ymax) {
		...
		nv.addGraph(function() {
			...
			.x(function (d) { return moment.utc(d.x).valueOf() })
